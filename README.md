...
| --- | --- | --- |
| *(a normal request)* | **main flow** (`SKILL.md`) | Decompose → route → assign a model → plan approval → dispatch → UI gate → retrospective |
| `start` / "begin" / "démarrer" / new feature | **`start`** (`START.md`) | Kick off a fresh feature: verify prerequisites (Orca, skills, tracker), then script `/grill-with-docs` → `/to-spec` → `/to-tickets` in one session, and hand over to the main flow with `/luucycle <parent ref>` |
